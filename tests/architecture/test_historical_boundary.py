...
class HistoricalBoundaryTests(unittest.TestCase):
    def test_historical_runtime_has_no_forbidden_authority_edges(self) -> None:
        prohibited = (
            "l9_debt_intelligence.snapshots",
            "l9_debt_intelligence.analytics",
            "l9_debt_intelligence.compilation",
            "l9_debt_intelligence.publication",
    ...
    def test_only_bootstrap_depends_on_existing_p1_service(self) -> None:
        importers = [
            path.name
            for path in SOURCE.rglob("*.py")
            if "l9_debt_intelligence.ingestion.service"
